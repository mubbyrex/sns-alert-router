...
- [x] Create directory structure:
  ...
  └── examples/standalone/
  ```
- [x] Confirm `modules/receivers/teams/` is NOT created. Its absence is a
      requirement (REQ-6), not an omission to fix.
- [x] `.gitignore` excludes at minimum: `*.tfstate`, `*.tfstate.*`,
      ...
      conventions shift).
- [x] `terraform init` succeeds in `modules/core/`, `modules/receivers/slack/`,
...
- [x] Add `severity_tiers` variable to `modules/core/variables.tf` exactly
      as specified in design.md (map of object, default
      critical/warning/info).
- [x] Create `aws_sns_topic` resources via `for_each` over `severity_tiers`
      ...
      must not collide).
- [x] Add `topic_arns` output (map of tier key → topic ARN) to
...
- [x] Add `aws_sns_topic_policy` resources, one per topic (via the same
      `for_each`), scoping `SNS:Publish` to `events.amazonaws.com` only, as
      shown in design.md.
- [x] Confirm no topic policy uses `Principal: "*"` or any wildcard broader
...
- [x] Check current `hashicorp/aws` provider documentation for the
...
- [x] Record findings as a short comment block at the top of
...
- [x] Create `modules/receivers/slack/variables.tf` with exactly the
...
- [x] Add a `validation` block (or `precondition`, whichever is more
      ...
      zero topics.
- [x] Create the Chatbot configuration resource in
...
- [x] Create the dedicated IAM role for this receiver: trust policy permits
      only the Chatbot service principal confirmed in Task 3; attach
      `guardrail_policy_arns`.
- [x] Add `chatbot_configuration_arn` output.
...
- [x] Create `modules/receivers/email/variables.tf` with `topic_arns`
      (map(string)), `severity_tiers` (list(string)), `email_addresses`
      (list(string)).
- [x] Create `aws_sns_topic_subscription` resources (protocol `"email"`)
      ...
      not prescriptive.
- [x] Add the same kind of `severity_tiers`-vs-`topic_arns` validation as
...
- [x] Document in this module's own short README/comment that SNS email
...
- [x] In `examples/standalone/`, add an EventBridge rule matching a
...
- [x] Demonstrate an `anything-but` exclusion as the noise-filtering
      technique.
- [x] Add an `input_transformer` reshaping the matched event into the
      minimum shape from requirements.md: `severity`, `source`, `title`,
      `description`, `timestamp`.
- [x] Target the transformed event at one of the topics from `modules.core`
...
- [x] `examples/standalone/main.tf` wires:
  ...
  - The EventBridge example pattern from Task 6, targeting `module.core`'s
    topic ARNs.
- [x] `examples/standalone/terraform.tfvars.example` provided with clearly
      ...
      team/channel IDs.
- [x] `examples/standalone/README.md`: brief — what this example proves
...
- [x] First section (before any usage instructions) states plainly what
...
- [x] Usage example mirrors `examples/standalone/`, showing the
      ...
      seen the design doc.
- [x] Explicitly states current scope: Slack and email receivers exist and
...
- [x] Roadmap section mentions Phase 2 (ECS reference integration) and
